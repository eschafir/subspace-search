...
def main():
    ...
    parser.add_argument("--split", type=str, default=None, help="Hugging Face dataset split/domain name (e.g. 'academia' for MM-BRIGHT, 'test' for ViDoRe)")
    parser.add_argument("--num-queries", type=int, default=10, help="Number of queries to process for distillation targets")
    ...
    # Check if we should load from Hugging Face or locally
    if args.dataset != "nfcorpus" and ("/" in args.dataset or args.dataset.startswith("vidore/")):
        print(f"Loading Hugging Face dataset: '{args.dataset}'...")
        try:
        ...
        if "mm-bright" in args.dataset.lower():
            # Specialized loader for MM-BRIGHT
            split_name = args.split if args.split else "academia"
            ...
            try:
                ...
                print(f"Error loading MM-BRIGHT from Hugging Face: {e}")
                return
            ...
            for idx, row in enumerate(ds):
                ...
                doc_id = row.get("doc_id") or row.get("image_filename") or f"DOC-{idx}"
        ...
        print(f"Parsed {len(queries)} queries and {len(corpus)} unique document pages.")
    else:
        # Load local NFCorpus
    ...
    # Filter queries with valid relevance judgments
    ...
    print(f"Generating teacher reasoning data for {len(selected_q_ids)} queries...")
    ...
    for idx, q_id in enumerate(selected_q_ids):
        ...
        print(f"\n--- Processing Query {idx+1}/{len(selected_q_ids)}: '{query_text}' ---")
